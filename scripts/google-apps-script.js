...
function berlinDayNumber(value){
 const p=Utilities.formatDate(new Date(value),'Europe/Berlin','yyyy-MM-dd').split('-').map(Number);
 return Math.floor(Date.UTC(p[0],p[1]-1,p[2])/86400000);
}
function eventMailWindow(data){
 const upcoming=(data.upcoming||data.meetings||[])
  .filter(t=>t&&t.id&&t.starts_at&&new Date(t.starts_at)>new Date())
  .sort((a,b)=>new Date(a.starts_at)-new Date(b.starts_at));
 if(!upcoming.length)return null;
 const anchor=upcoming[0],days=berlinDayNumber(anchor.starts_at)-berlinDayNumber(new Date());
 if(days!==7&&days!==2)return null;
 return {anchor:anchor,days:days,key:(anchor.kind||'meeting')+'_'+anchor.id+'_'+days};
}
...
function buildMail(member,data,kind,window){
 ...
 const lead=window&&window.days===2?'In 2 Tagen':'In einer Woche';
 const subject=window?lead+': '+window.anchor.title+' – dein Wochenausblick':'Dein Wochenausblick – nächste Treffen & Events';
 return {subject:subject,text:info,html:frame(greeting+textBlock('Das steht als Nächstes an.',26)+textBlock('Die nächsten drei Treffen & Events – für deine Planung.',15,'#c4beb4')+(cards||panel(textBlock(info)))+textBlock('Details und Rückmeldungen findest du in der App.',14,'#c4beb4'))};
 }
 ...
 const banner=panel(textBlock(outstanding?'NOCH OFFEN':'ALLES BEZAHLT',12,outstanding?'#ffd166':'#78dba0')+textBlock(sum(open),38,outstanding?'#ffd166':'#78dba0')+textBlock(outstanding?'Deine Strafenübersicht':'Alles erledigt – danke!',22)+textBlock(outstanding?'Hier findest du deine offenen Beträge und bisherigen Zahlungen.':'Du hast aktuell keine offenen Strafen. Damit bist du für den nächsten Termin auf dem aktuellen Stand.',15,'#f7f3ea'),outstanding?'#332710':'#173326');
 ...
 const eventNote=window?panel(textBlock(window.days===2?'NÄCHSTER TERMIN IN 2 TAGEN':'NÄCHSTER TERMIN IN EINER WOCHE',12,'#ffd166')+textBlock(window.anchor.title,20)+textBlock(Utilities.formatDate(new Date(window.anchor.starts_at),'Europe/Berlin','dd.MM.yyyy HH:mm')+' Uhr',15,'#c4beb4')):'';
 return {subject:outstanding?'Offen: '+sum(open)+' – deine Zahlungsübersicht':'Alles bezahlt – deine Zahlungsübersicht',text:'Hallo '+member.name+', offen: '+sum(open)+'. Bereits bezahlt: '+sum(paid)+'. Die Zahlungsübersicht liegt bei.',
 html:frame(greeting+eventNote+banner+(open.length?textBlock('Offene Strafen · '+open.length,22)+table(open,false):'')+textBlock('Bereits bezahlt · '+sum(paid),22)+(paid.length?table(paid,true):textBlock('Noch keine bezahlten Strafen erfasst.',15,'#c4beb4'))+textBlock('Deine druckbare Zahlungsübersicht findest du im Anhang.',14,'#c4beb4')),
 attachment:Utilities.newBlob(receipt,'text/html','Zahlungsuebersicht.html')};
}
function runWeekly(kind,preview){
 const lock=LockService.getScriptLock();lock.waitLock(30000);
 try{
 const data=getMailData(),props=PropertiesService.getScriptProperties(),window=eventMailWindow(data);
 // Vorschauen sind jederzeit möglich; der echte Versand nur 7 oder 2 Kalendertage vor dem nächsten Termin.
 if(!preview&&!window)return;
 data.members.forEach(m=>{
 const mail=buildMail(m,data,kind,window);if(!mail)return;
 const marker='sent_'+kind+'_'+m.id;if(!preview&&props.getProperty(marker)===window.key)return;
 ...
 GmailApp.sendEmail(m.email,mail.subject,mail.text,options);props.setProperty(marker,window.key);}
 });
...
function sendScheduledEventEmails(){runWeekly('fine',false);runWeekly('meeting',false);}
...
function createWeeklyTrigger(){
 ScriptApp.getProjectTriggers().forEach(t=>{if(['sendWeeklyFineEmails','sendWeeklyMeetingEmails','sendScheduledEventEmails','sendPendingProtocolEmails'].includes(t.getHandlerFunction()))ScriptApp.deleteTrigger(t);});
 ScriptApp.newTrigger('sendScheduledEventEmails').timeBased().everyDays(1).atHour(9).nearMinute(30).inTimezone('Europe/Berlin').create();
