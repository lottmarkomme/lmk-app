(() => {
  ...
  const state = { mode: 'login', authBusy: false, user: null, profile: null, profiles: [], fines: [], catalog: [], meeting: null, attendance: [] };
  ...
  async function init() {
    ...
    client.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_IN' && session?.user && !state.authBusy && !$('auth-view').classList.contains('hidden')) window.setTimeout(() => enterApp(session.user), 0);
      if (!session) { $('app-view').classList.add('hidden'); $('auth-view').classList.remove('hidden'); }
    });
